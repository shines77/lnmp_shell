...
Install_Apache_22()
{
    ...
    if [ "${Stack}" = "lamp" ]; then
        ...
        \cp ${cur_dir}/conf/httpd22-lnamp.conf /usr/local/apache/conf/httpd.conf
        \cp ${cur_dir}/conf/httpd-vhosts-lnamp.conf /usr/local/apache/conf/extra/httpd-vhosts.conf
    fi
...
Install_Apache_24()
{
    ...
    if [ "${Stack}" = "lamp" ]; then
        ...
        \cp ${cur_dir}/conf/httpd24-lnamp.conf /usr/local/apache/conf/httpd.conf
        \cp ${cur_dir}/conf/httpd-vhosts-lnamp.conf /usr/local/apache/conf/extra/httpd-vhosts.conf
    fi
    \cp ${cur_dir}/conf/httpd-default.conf /usr/local/apache/conf/extra/httpd-default.conf
    \cp ${cur_dir}/conf/mod_remoteip.conf /usr/local/apache/conf/extra/mod_remoteip.conf
